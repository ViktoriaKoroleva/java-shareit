...
public class BookingServiceImpl implements BookingService {
    ...
    public BookingDto saveBooking(Long userId, BookingRequestDto bookingRequestDto) {
        Item item = itemRepository.findById(bookingRequestDto.getItemId())
                .orElseThrow(() -> new NotFoundExceptions("Такой вещи для бронирования нет"));
        User user = userRepository.findById(userId)
                .orElseThrow(() -> new NotFoundExceptions("Пользователь с id = " + userId + " не найден"));
    ...
    public List<BookingDto> getAllByBookerAndStatus(Long userId, String state) {
        userRepository.findById(userId)
                .orElseThrow(() -> new NotFoundExceptions("Пользователь не найден"));
